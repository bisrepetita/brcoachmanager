...
// cartes ont un fond propre, pour s'intégrer à n'importe quel design). Ne réalise jamais d'action
...
export default function EmbedPlanningPage() {
  ...
  return (
    <div style={{ background: 'transparent', padding: '8px 4px', fontFamily: 'var(--font-sans, sans-serif)' }}>
      {loading ? (
                  ...
                  return (
                    <a
                      ...
                      style={{
                        ...
                        border: '1px solid #EEEAE3', boxShadow: '0 1px 2px rgba(26,26,24,0.04)',
                        cursor: 'pointer', textDecoration: 'none',
                        display: 'flex', alignItems: 'stretch', gap: 12,
                      }}
                    >
                      <div style={{
                        flexShrink: 0, width: 52, borderRadius: 10, background: '#F5F3F0',
                        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '8px 0',
                      }}>
                        <p style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#1A1A18', lineHeight: 1.2, fontVariantNumeric: 'tabular-nums' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 5 }}>
                        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                          <p style={{ fontSize: 15, fontWeight: 600, color: '#1A1A18', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, minWidth: 0 }}>
                            {gs.title}
                          </p>
                          {gs.level && (
                            <span style={{ fontSize: 10, fontWeight: 600, color: '#7A7570', background: '#F5F3F0', padding: '2px 7px', borderRadius: 20, whiteSpace: 'nowrap', flexShrink: 0 }}>
                        ...
                        {gs.coachNames && gs.coachNames.length > 0 && (
                          <p style={{ margin: 0, fontSize: 12, color: '#7A7570', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            Avec {gs.coachNames.join(' & ')}
                          </p>
                        )}

                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginTop: 2 }}>
                          <span style={{ fontSize: 13, fontWeight: 600, color: '#1A1A18', fontFamily: 'monospace' }}>
                            {gs.price.toFixed(2)} CHF
                          </span>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            {isFull ? (
                              <span style={{ fontSize: 11, fontWeight: 600, color: '#C0392B' }}>Complet</span>
                            ) : (
                              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                <Users2 size={12} color="#7A7570" />
                                <span style={{ fontSize: 12, color: '#7A7570', fontVariantNumeric: 'tabular-nums' }}>
                                  {spotsLeft} place{spotsLeft > 1 ? 's' : ''}
                                </span>
                              </div>
                            )}
                            <span style={{ display: 'flex', alignItems: 'center', gap: 2, fontSize: 12, fontWeight: 600, color: '#1A1A18' }}>
